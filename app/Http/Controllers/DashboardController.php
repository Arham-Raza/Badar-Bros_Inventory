    ...
    public function index()
    {
        try {
            ...
            if ($firstSale || $firstPurchase || $firstReceipt || $firstPayment) {
                $saleMonthsCount = $firstSale ? Carbon::now()->diffInMonths(Carbon::parse($firstSale->created_at)) + 1 : 0;
                $purchaseMonthsCount = $firstPurchase ? Carbon::now()->diffInMonths(Carbon::parse($firstPurchase->created_at)) + 1 : 0;
                $receiptMonthsCount = $firstReceipt ? Carbon::now()->diffInMonths(Carbon::parse($firstReceipt->created_at)) + 1 : 0;
                $paymentMonthsCount = $firstPayment ?Carbon::now()->diffInMonths(Carbon::parse($firstPayment->created_at)) + 1 : 0;
            }
